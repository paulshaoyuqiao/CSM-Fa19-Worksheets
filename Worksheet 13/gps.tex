...
\begin{enumerate}
    ...
    \sol {
        ...
        \begin{align*}
            \norm{\vec{x} - \vec{p}_{1}}^{2} &= \vec{x}^{T} \vec{x} - 2 \vec{p}_{1}^{T} \vec{x} + \vec{p}_{1}^{T} \vec{p}_{1} = d_{1}^{2} \\
            &\vdots \\
            \norm{\vec{x} - \vec{p}_{m}}^{2} &= \vec{x}^{T} \vec{x} - 2 \vec{p}_{m}^{T} \vec{x} + \vec{p}_{m}^{T} \vec{p}_{m} = d_{m}^{2} 
        \end{align*}
        ...
        \begin{align*}
            2 \vec{p}_{1}^{T} \vec{x} - 2 \vec{p}_{2}^{T} \vec{x} &= d_{2}^{2} - d_{1}^{2} + \vec{p}_{2}^{T} \vec{p}_{2} - \vec{p}_{1}^{T} \vec{p}_{1} \\
            &\vdots \\
            2 \vec{p}_{1}^{T} \vec{x} - 2 \vec{p}_{m}^{T} \vec{x} &= d_{m}^{2} - d_{1}^{2} + \vec{p}_{m}^{T} \vec{p}_{m} - \vec{p}_{1}^{T} \vec{p}_{1}
        \end{align*}
    ...
    \sol {
        The least-squares solution will be $\vec{\hat{v}} = (A^{T}A)^{-1} A^{T} \vec{b}.$
    }
    ...
    \sol {
        ...
        We then create an estimate $\vec{\hat{r}} = \sum\limits_{l = 1}^{k} \alpha_{l} \vec{s}_{l}^{\ (\tau_{l})}$ using the information we have. 
        The stopping condition of OMP will be when the residual $\vec{e} = \vec{r} - \vec{\hat{r}}$ is smaller than some threshold for us to say that we have explained the received signal $\vec{r}$ to the best of our ability.
    }
