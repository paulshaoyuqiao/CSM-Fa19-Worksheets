...
\begin{enumerate}
    ...
    \sol{
        ...
        We cross-correlate $\vec{e}$ with each $\vec{s}_i$:
        \begin{itemize}
            \item $\texttt{circcorr}(\vec{e}, \vec{s}_1) = \begin{bmatrix} 1.68 \\ -0.9 \\ -0.78 \end{bmatrix}$
            \item $\texttt{circcorr}(\vec{e}, \vec{s}_2) = \begin{bmatrix} -0.33 \\  0.21 \\ 0.12 \end{bmatrix}$
            \item $\texttt{circcorr}(\vec{e}, \vec{s}_3) = \begin{bmatrix} -0.9 \\ 1.08 \\ -0.18 \end{bmatrix}$
        \end{itemize}
        The largest magnitude is in the correlation with $\vec{s}_1$ at index 0.
    ...
    \sol{
        ...
        \begin{itemize}
            \item $\texttt{circcorr}(\vec{e}, \vec{s}_2) = \begin{bmatrix} 0.088 \\ 0.092 \\ 0.018 \end{bmatrix}$
            \item $\texttt{circcorr}(\vec{e}, \vec{s}_3) = \begin{bmatrix} -0.231 \\ -0.218 \\ -0.242 \end{bmatrix}$
        \end{itemize}
        Index 2 of the correlation with $\vec{s}_3$ has the largest value (keep in mind that correlation is zero-indexed).
    }
    ...
    We do least squares again, this time with $A = \begin{bmatrix} \vline & \vline \\ \vec{s}_1^{(0)} & \vec{s}_3^{(2)} \\ \vline & \vline \end{bmatrix}$.
        $$
            \vec{x}_{curr} = (\begin{bmatrix} -0.5 & 0.5 & 0.1 \\ -0.6 & 0 & 0 \end{bmatrix}
            \begin{bmatrix} -0.5 & -0.6 \\ 0.5 & 0 \\ 0.1 & 0 \end{bmatrix})^{-1}
            \begin{bmatrix} -0.5 & 0.5 & 0.1 \\ -0.6 & 0 & 0 \end{bmatrix}
            \begin{bmatrix} -1.8 \\ 1.5 \\ 0.3 \end{bmatrix}
    ...
    \item Express $\vec{r}$ as a linear combination of $\vec{s}_1$, $\vec{s}_2$, and $\vec{s}_3$.
    What is our final error vector $\vec{e}$?
    \\ \\
    \sol{
        The result of OMP tells us that $\vec{r} = 3\vec{s}_1^{(0)} + 0.5\vec{s}_2^{(2)}$.
        Our error vector $\vec{r} - A_{curr}\vec{x}_{curr}$ turns out to be exactly $\vec{0}$.
        Note that if we had received some noise, i.e.
        $$\vec{r} = 3\vec{s}_1^{(0)} + 0.5\vec{s}_2^{(2)} + \vec{c}$$
        for some small $\vec{c}$, then we would instead have $\vec{e} = \vec{c}$.
    }
    \\ \\
    \note{
